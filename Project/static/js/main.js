...
async function loadInitialData() {
  try {
    ...
    console.warn("Could not load presets from server, using built-in defaults.", err);
    loadDefaultAimlPreset();
  }
}

// Load a specific preset
function loadPreset(presetKey) {
  if (!window.cachedPresets || !window.cachedPresets[presetKey]) {
    loadDefaultAimlPreset();
    return;
  }

  const p = window.cachedPresets[presetKey];
  ...
  // Sync inputs
  document.getElementById("availableHours").value = appState.availableHours;
  document.getElementById("hoursValDisplay").textContent = `${appState.availableHours.toFixed(1)} hrs`;
  document.getElementById("startTime").value = appState.startTime;
  document.getElementById("sessionLength").value = appState.sessionLength;
...
// Fallback AIML preset generator
function loadDefaultAimlPreset() {
